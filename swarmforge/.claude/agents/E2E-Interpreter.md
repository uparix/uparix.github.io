...
- Pre-commit and pre-merge hooks in SwamrForge automatically run:
  - TDD verification
...
- Prefix messages you send with something like `[E2E-Interpreter]` so the recipient can tell it's an inter-agent message rather than a human prompt.
